...
// tflite_flutter 等插件的 android 模块自身未设置 Java/Kotlin 的 JVM target：
// Java 落回默认 1.8，而 Kotlin 编译器跟随当前 JDK 走到 21，触发 Gradle 的
// "Inconsistent JVM-target compatibility" 校验直接构建失败。
// 在插件自身配置生效之后，把所有 library 模块统一到 17（与 app 模块一致）。
fun Project.unifyJvmTarget() {
    plugins.withId("com.android.library") {
